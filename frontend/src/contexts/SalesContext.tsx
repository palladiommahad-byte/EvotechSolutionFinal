...
export interface SalesItem {
  ...
  description: string;
  quantity: number;
  unit?: string;     // Unit of measure (e.g. Piece, kg, litre) — optional
  unitPrice: number;
  total: number;
}
...
interface SalesContextType {
  ...
  createPrelevement: (data: Omit<SalesDocument, 'id' | 'type'>) => Promise<any>;
  ...
  // Grouped BL → Invoice creation
  createInvoiceFromBLs: (payload: {
    bl_ids: string[];
    date: string;
    due_date?: string;
...
const deliveryNoteToSalesDocument = (deliveryNote: DeliveryNoteWithItems): SalesDocument => {
  ...
  return {
    ...
    // Tax flag — DB column defaults to true; undefined/null also treated as true
    taxEnabled: (deliveryNote as any).tax_enabled !== false,
    clientPoNumber: deliveryNote.client_po_number || undefined,
...
export const SalesProvider = ({ children }: { children: ReactNode }) => {
  ...
  // ── Mutation: Create invoice from selected BLs (grouped invoicing) ──────
  const createInvoiceFromBLsMutation = useMutation({
    mutationFn: async (payload: {
      ...
      return invoicesService.createFromBLs(payload);
    },
    onSuccess: (invoice) => {
      queryClient.invalidateQueries({ queryKey: ['sales', 'invoices'] });
      queryClient.invalidateQueries({ queryKey: ['sales', 'deliveryNotes'] });
  ...
  const value: SalesContextType = {
    ...
    creditNotes,
    isLoading,
    createInvoice: async (data) => { return await createInvoiceMutation.mutateAsync(data); },
    ...
    createEstimate: async (data) => { return await createEstimateMutation.mutateAsync(data); },
    ...
    createDivers: async (data) => { return await createDiversMutation.mutateAsync(data); },
    ...
    createCreditNote: async (data) => { return await createCreditNoteMutation.mutateAsync(data); },
    ...
    prelevements,
    createPrelevement: async (data) => { return await createPrelevementMutation.mutateAsync(data); },
    ...
    refreshAll,
    createInvoiceFromBLs: async (payload) => { return await createInvoiceFromBLsMutation.mutateAsync(payload); },
  };
