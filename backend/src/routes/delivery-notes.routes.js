...
router.post('/', asyncHandler(async (req, res) => {
    ...
    if (!document_id) {
        try {
            document_id = await generateDocumentNumber(document_type, date);
        } catch (err) {
    ...
    try {
       ...
       RETURNING *`,
        ...
        for (const item of items) {
            ...
            await client.query(
                `INSERT INTO delivery_note_items (delivery_note_id, product_id, description, quantity, unit, unit_price, total)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
                [deliveryNote.id, item.product_id || null, item.description, item.quantity, item.unit || null, item.unit_price, item.quantity * item.unit_price]
            );
